...
class _RFCN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic):
        ...
        # self.RFCN_psroi_loc_pool = _PSRoIPooling(cfg.POOLING_SIZE, cfg.POOLING_SIZE,
        #                                          spatial_scale=1.0 / 16.0, group_size=7,output_dim=4)
        ############################################################################
        # self.RFCN_match_channel = nn.Conv2d(4*self.n_reg_classes, 4, kernel_size=3, stride=1, padding=1)
        ############################################################################
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes):
        ...
        for i_leg in range(num_legs):
            ...
            base_feats = self._im_to_head(leg_im_data) #leg_im_data 1*3*600*1067   base_feats
            base_conv3.append(base_feats[0])    #1*512*75*100
            base_conv4.append(base_feats[1])    #1*1024*38*50
            base_conv5.append(base_feats[2])    #1*2048*38*50
            base_feat.append(base_feats[3])     #1*512*38*50
            
            rfcn_cls.append(self.RFCN_cls_net(base_feat[i_leg]))  #1*1519*38*50
            rfcn_bbox.append(self.RFCN_bbox_net(base_feat[i_leg]))   #1*6076*38*50
            # feed base feature map tp RPN to obtain rois
            rpn_out = self.RFCN_rpn(base_feat[i_leg], 
                                    im_info[i_leg], 
                                    gt_boxes[i_leg][:,:,:5], 
                                    num_boxes[i_leg])  #rois  rois(obj & non-obj) in image with score after nms  & clsloss bboxloss
            rois.append(rpn_out[0])   #rois
            ...
            leg_pooled_cls_feat = self.RFCN_psroi_cls_pool(rfcn_cls[i_leg], rois[i_leg].view(-1,5))#128*34*7*7
            leg_pooled_loc_feat = self.RFCN_psroi_loc_pool(rfcn_bbox[i_leg], rois[i_leg].view(-1,5))#128*124*7*7
            # compute object classification probability
            cls_score.append(self.RFCN_cls_score(leg_pooled_cls_feat).squeeze())
            cls_prob.append(F.softmax(cls_score[i_leg], dim=1))#128*31  final prob

            # compute bbox offset
            bbox_pred.append(self.RFCN_bbox_pred(leg_pooled_loc_feat).squeeze())  #128*31*4  offsets
            ...
            # self.RFCN_loss_cls.append(torch.zeros(1).cuda())
            # self.RFCN_loss_bbox.append(torch.zeros(1).cuda())

            if self.training:
                # classification loss
                RFCN_loss_cls[i_leg] = F.cross_entropy(cls_score[i_leg], leg_rois_label)
                # self.RFCN_loss_cls[i_leg] = F.cross_entropy(cls_score[i_leg], leg_rois_label)
                # bounding box regression L1 loss
                RFCN_loss_bbox[i_leg] = _smooth_l1_loss(bbox_pred[i_leg],
                                                        leg_rois_target, leg_rois_inside_ws, leg_rois_outside_ws)
                # self.RFCN_loss_bbox[i_leg] = _smooth_l1_loss(bbox_pred[i_leg],
                #     leg_rois_target, leg_rois_inside_ws, leg_rois_outside_ws)
        ...
        tracking_feat = torch.cat(tracking_feature_list, dim=1)  #1*1051*38*50
        tracking_reg_coords = self.corr_bbox_net(tracking_feat)   #1051->6076
        ...
        tracking_pred = self.RFCN_tracking_pred(psroi_pooled_tracking_rois).permute(2,1,0,3)
        tracking_pred = self.RFCN_match_channel(tracking_pred)
        tracking_pred = tracking_pred.squeeze().permute(1,0)  #30*124-->30*4
        if self.training:
        ...
        rois0 = rois[0].unsqueeze(0)
        rois1 = rois[1].unsqueeze(0)
        rois = torch.cat((rois0, rois1), dim=0)
        # cls_prob = map(lambda x:x.unsqueeze(0), cls_prob)
        # cls_prob = torch.cat(cls_prob, dim=0)
        cls_prob0 = cls_prob[0].unsqueeze(0)
        cls_prob1 = cls_prob[1].unsqueeze(0)
        cls_prob = torch.cat((cls_prob0, cls_prob1), dim=0)
        # bbox_pred = map(lambda x:x.unsqueeze(0), bbox_pred)
        # bbox_pred = torch.cat(bbox_pred, dim=0)
        bbox_pred0 = bbox_pred[0].unsqueeze(0)
        bbox_pred1 = bbox_pred[1].unsqueeze(0)
        bbox_pred = torch.cat((bbox_pred0, bbox_pred1), dim=0)
        ...
        if len(rois_label)>0:
            ...
            rois_label0 = rois_label[0].unsqueeze(0)
            rois_label1 = rois_label[1].unsqueeze(0)
            rois_label = torch.cat((rois_label0, rois_label1), dim=0)
        ...
        rpn_loss_cls0 = rpn_loss_cls[0].unsqueeze(0)
        rpn_loss_cls1 = rpn_loss_cls[1].unsqueeze(0)
        rpn_loss_cls = torch.cat((rpn_loss_cls0, rpn_loss_cls1), dim=0)
        # rpn_loss_bbox = map(lambda x:x.unsqueeze(0), rpn_loss_bbox)
        # rpn_loss_bbox = torch.cat(rpn_loss_bbox, dim=0)
        rpn_loss_bbox0 = rpn_loss_bbox[0].unsqueeze(0)
        rpn_loss_bbox1 = rpn_loss_bbox[1].unsqueeze(0)
        rpn_loss_bbox = torch.cat((rpn_loss_bbox0, rpn_loss_bbox1), dim=0)
        # RFCN_loss_cls = map(lambda x:x.unsqueeze(0), RFCN_loss_cls)
        # RFCN_loss_cls = torch.cat(RFCN_loss_cls, dim=0)
        RFCN_loss_cls0 = RFCN_loss_cls[0].unsqueeze(0)
        RFCN_loss_cls1 = RFCN_loss_cls[1].unsqueeze(0)
        RFCN_loss_cls = torch.cat((RFCN_loss_cls0, RFCN_loss_cls1), dim=0)
        # RFCN_loss_bbox = map(lambda x:x.unsqueeze(0), RFCN_loss_bbox)
        # RFCN_loss_bbox = torch.cat(RFCN_loss_bbox, dim=0)
        RFCN_loss_bbox0 = RFCN_loss_bbox[0].unsqueeze(0)
        RFCN_loss_bbox1 = RFCN_loss_bbox[1].unsqueeze(0)
        RFCN_loss_bbox = torch.cat((RFCN_loss_bbox0, RFCN_loss_bbox1), dim=0)
