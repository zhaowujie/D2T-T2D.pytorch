...
class resnet(_RFCN):
  ...
  def _init_modules(self):
    ...
    # self.corr_bbox_net = nn.Conv2d(12811, 4 * self.n_reg_classes * 7 * 7, [1, 1], padding=0, stride=1)
    ####################################################################################################
    self.RFCN_match_channel = nn.Conv2d(4 * self.n_reg_classes, 4, kernel_size=3, stride=1, padding=1)
    nn.init.normal(self.RFCN_match_channel.weight.data, 0.0, 0.01)
    ####################################################################################################
  ...
  def _im_to_head(self, x):
    ...
    conv3 = self.RFCN_base[5](conv2)  #1*512*75*100
    conv4 = self.RFCN_base[6](conv3)  #1*1024*38*50
    conv5 = self.RFCN_base[7](conv4)  #1*2048*38*50
    top_feat = self.RFCN_base[8](conv5)
    top_feat = self.RFCN_base[9](top_feat)  #1*512*38*50
