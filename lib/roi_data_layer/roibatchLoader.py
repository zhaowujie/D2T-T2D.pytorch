...
class roibatchLoader(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    for ientry, entry in enumerate(minibatch_db):
        ...
        if self.training:
            ...
            if keep.numel() != 0:
                ...
                num_boxes.append(torch.LongTensor([min(gt_boxes[ientry].size(0), self.max_num_box)]))
    ...
    if self.training:
        ...
        gt_boxes_padding_pair = torch.cat(gt_boxes_padding, dim=0)   #gt   (x1,y1,x2,y2, cls, frame_id)
