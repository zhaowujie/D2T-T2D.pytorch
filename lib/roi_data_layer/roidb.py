#coding=utf-8 """Transform a roidb into a trainable roidb by adding a bunch of metadata."""
...
def combined_roidb(imdb_names, training=True, duplicate_frames=False):
  ...
  def get_roidb(imdb_name):
    ...
    roidb = get_training_roidb(imdb)     #roidb: all gt bbox
    return roidb
  ...
  if training:
    roidb = filter_roidb(roidb)  #filter img without bbox
  roidb_pairs = create_roi_pairs(roidb, training, duplicate_frames)  #roi pairs  得到配对的roi
  if training:
    np.random.seed(123)
    np.random.shuffle(roidb_pairs)
  ratio_list, ratio_index = rank_roidb_ratio(roidb_pairs)  #get image ratio and sort them  from little to big saved in index
  # if not os.path.exists('/media/hp208/4t/zhaoxingjie/graduation_project/d2t/pytorch-detect-to-track/data/imdb.pkl'):
  #     import pickle
  #     imdb_path = '/media/hp208/4t/zhaoxingjie/graduation_project/d2t/pytorch-detect-to-track/data/imdb.pkl'
  #     roidb_pairs_path = '/media/hp208/4t/zhaoxingjie/graduation_project/d2t/pytorch-detect-to-track/data/roidb_pairs.pkl'
  #     ratio_list_path = '/media/hp208/4t/zhaoxingjie/graduation_project/d2t/pytorch-detect-to-track/data/ratio_list.pkl'
  #     ratio_index_path = '/media/hp208/4t/zhaoxingjie/graduation_project/d2t/pytorch-detect-to-track/data/ratio_index.pkl'
  #     with open(imdb_path, 'wb')as f1:
  #       pickle.dump(imdb, f1)
  #     with open(roidb_pairs_path, 'wb')as f2:
  #       pickle.dump(roidb_pairs, f2)
  #     with open(ratio_list_path, 'wb')as f3:
  #       pickle.dump(ratio_list, f3)
  #     with open(ratio_index_path, 'wb')as f4:
  #       pickle.dump(ratio_index, f4)
